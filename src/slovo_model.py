...
class SlovoONNXModel:
    ...
    def infer(
        ...
        *,
        ...
        """
        should_preprocess = self.config.enable_preprocessing if preprocess is None else preprocess
    ...
    def _prepare_model_input(
        ...
        *,
        ...
        if preprocess:
            if data.ndim != 4:
                raise ValueError("Preprocessing expects raw frames in shape (T, H, W, C).")
    ...
    def _to_model_layout(self, data: np.ndarray) -> np.ndarray:
        ...
        if data.ndim != 4:
            raise ValueError(
                f"Expected model input with 4 or 5 dims, got shape {tuple(data.shape)}."
            )

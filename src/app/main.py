...
def _build_aggregator() -> SentenceAggregator:
    ...
    if settings.llm_provider == "openai":
        if not settings.llm_model or not settings.llm_api_key:
            raise RuntimeError("Set NONVERBAL_LLM_MODEL and NONVERBAL_LLM_API_KEY for OpenAI mode.")
...
async def recognize(request: Request) -> RecognizeResponse:
    ...
    service: RecognitionService | None = getattr(request.app.state, "recognition_service", None)
    if service is None:
        raise HTTPException(
            status_code=503,

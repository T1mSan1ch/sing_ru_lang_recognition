...
class RecognitionService:
    ...
    def _infer_and_merge_clips(
        ...
        clip_inferences = [self._model.infer(clip, preprocess=False) for clip in clips]
    ...
    def _merge_single_output(
        *,
        ...
        squeezed = [RecognitionService._drop_leading_batch_axis(item) for item in outputs]
        ...
        merged_len = max(
            start + length for start, length in zip(scaled_starts, lengths, strict=True)
        )
